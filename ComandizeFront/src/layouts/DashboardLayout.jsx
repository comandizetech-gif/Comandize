...
function getPageFromHash() {
  ...
  const exists = allMenuItems.some((item) => item.name === hash);
  return exists ? hash : "Delivery";
}

function setPageHash(pageName) {
  const nextHash = encodeURIComponent(pageName);
...
function DashboardLayout() {
  ...
  useEffect(() => {
    ...
    if (!window.location.hash) {
      window.history.replaceState(null, "", `${window.location.pathname}${window.location.search}#Delivery`);
    }
  ...
  const canViewPage = (pageName) => {
    ...
    return Boolean(
      permissions.find((item) => item.page === pageName && item.canView)
    );
  };

  const menuItems = useMemo(
    () => allMenuItems.filter((item) => canViewPage(item.name)),
    [user]
  );

  useEffect(() => {
    if (!checkingSession && menuItems.length > 0 && !canViewPage(activePage)) {
      const firstAllowedPage = menuItems[0].name;
      setActivePage(firstAllowedPage);
      setPageHash(firstAllowedPage);
    }
  ...
  if (checkingSession) {
    return (
      <div className="min-h-screen bg-[#f5f6fa] flex items-center justify-center text-[#374151] font-black">
        Validando sessão...
      </div>
    );
  }
      ...
      >
        <div className="flex justify-center py-4 border-b border-gray-200 relative">
          ...
          <button
            type="button"
            onClick={() => setSidebarOpen(false)}
            className="lg:hidden absolute right-4 top-4 w-10 h-10 rounded-xl bg-[#fff7ed] text-[#ff8a00] flex items-center justify-center"
        ...
        <header className="h-16 lg:h-20 bg-white border-b border-[#e5e7eb] flex items-center justify-between px-4 lg:px-8 shadow-sm sticky top-0 z-30">
          ...
          <div className="flex items-center gap-3 shrink-0">
            <div className="hidden sm:block text-right">
              <p className="font-bold text-[#374151] leading-tight">
                {user?.name || "Usuário"}
              </p>
              <p className="text-xs text-[#ff8a00] font-semibold">
                {user?.isSubAccount ? "Subconta ativa" : "Conta principal"}
              </p>
            </div>
